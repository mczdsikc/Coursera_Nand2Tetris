...
class CodeWriter(object):
    ...
    def parse(self, command):
        ...
        if command[0] == VMConstant.C_ARITHMETIC:
            ...
            if command[1] == VMConstant.add:
                self._add()
            elif command[1] == VMConstant.sub:
                self._sub()
            elif command[1] == VMConstant.neg:
                self._neg()
            elif command[1] == VMConstant.eq:
                self._eq()
            elif command[1] == VMConstant.gt:
                self._gt()
            elif command[1] == VMConstant.lt:
                self._lt()
            elif command[1] == VMConstant.and_:
                self._and()
            elif command[1] == VMConstant.or_:
                self._or()
            elif command[1] == VMConstant.not_:
                self._not()
    ...
    def _return(self):
        self._pop()
        self._a_instr('R13')
        self._c_instr('M', 'D')     # return value -> R13
        self._a_instr(VMConstant.local.value)
        self._c_instr('D', 'M')
        self._a_instr(VMConstant.SP.value)
        self._c_instr('M', 'D')     # SP = LCL
        for value in [VMConstant.that.value, VMConstant.this.value]:
            self._pop()
            self._a_instr(value)
            self._c_instr('M', 'D') # restore THAT, THIS
        self._pop()
        self._a_instr('R14')
        self._c_instr('M', 'D')     # ARG -> R14
        self._pop()
        self._a_instr(VMConstant.local.value)
        self._c_instr('M', 'D')     # restore LCL
        self._pop()
        self._a_instr('R15')
        self._c_instr('M', 'D')     # return address -> R15
        self._a_instr('R13')
        self._c_instr('D', 'M')
        self._a_instr(VMConstant.argument.value)
        self._c_instr('A', 'M')
        self._c_instr('M', 'D')     # set return value -> ARG[0]
        self._c_instr('D', 'A+1')
        self._a_instr(VMConstant.SP.value)
        self._c_instr('M', 'D')     # SP = ARG + 1
        self._a_instr('R14')
        self._c_instr('D', 'M')
        self._a_instr(VMConstant.argument.value)
        self._c_instr('M', 'D')     # restore ARG
        self._a_instr('R15')
        self._c_instr('A', 'M')     # A = return address
        self._c_instr('', '0', 'JMP')

        # this sequence is same in book
        # self._a_instr(VMConstant.local.value)
    ...
    def _sp_add(self, arg='+1'):
        '(stack pointor) SP++, or SP-- if arg=="-1"'
        self._a_instr('SP')        # @SP
        self._c_instr('M', 'M' + arg) # M=M+1 or M=M-1
